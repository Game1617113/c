...
def clean_expression_for_eval(expression):
    """
    ลบศูนย์นำหน้าออกจากตัวเลขในนิพจน์ โดยไม่ใช้ regex
    ตัวอย่าง: "01+005" -> "1+5", "01.05" -> "1.05"
    """
    cleaned_parts = []
    num_buf = []
    def flush_number_buf():
        ...
        if '.' in token:
            ...
            if int_part == '':
                int_part = '0'
            token = int_part + '.' + frac_part
        else:
            # ลบศูนย์นำหน้าทั้งหมด แต่ให้ค่า '0' อย่างน้อยหนึ่งตัว
    ...
    for ch in expression:
        # ให้ตัวเลขและจุดเป็นส่วนของ token ตัวเลข
        if ch.isdigit() or ch == '.':
            num_buf.append(ch)
        else:
            flush_number_buf()
